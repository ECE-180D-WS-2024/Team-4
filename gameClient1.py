...
while True:
    ...
    ##########################################
    #This is the part that manages the socket communication

    if(not player1_turn):
        ####################################################################
        #SEND CLAUSE
        if (line != None):
            sendData = make_pos((ballStationary[0],ballStationary[1], strokes_1, h1, int(player1_turn), int(player1attacked), int(player2attacked), int(powerUpPlayer1), line[0], line[1], int(shoot)))
        else:
            sendData = make_pos((ballStationary[0],ballStationary[1], strokes_1, h1, int(player1_turn), int(player1attacked), int(player2attacked), int(powerUpPlayer1), -1, -1, int(shoot)))
        
        recData = player1client.send(sendData)
        ####################################################################

        player2data = read_pos(recData)

        #Key Variables (x,y, strokes, h2, player1_turn, player1attacked, player2attacked, powerupplayer2 , linex, liney, shoot)
        ballStationary2 = (player2data[0], player2data[1])
        h2 = player2data[3]
        ##If there is an addition in strokes and you arent in the hole, change turns

        if (strokes_2 < player2data[2] and h1 != 1 and player2data[10] == 0):
            player1_turn = True

        strokes_2 = player2data[2]
        #player1_turn = bool(player2data[4])
        player1attacked = bool(player2data[5])
        player2attacked = bool(player2data[6])
        powerUpPlayer2 = bool(player2data[7])
        if (player2data[8] == -1 and player2data[9] == -1):
            line = None
        else:
            line = (player2data[8], player2data[9])
        shoot = bool(player2data[10])
    ...
    #SEND CLAUSE
    if (line != None):
        sendData = make_pos((ballStationary[0],ballStationary[1], strokes_1, h1, int(player1_turn), int(player1attacked), int(player2attacked), int(powerUpPlayer1), line[0], line[1],  False))
    else:
        sendData = make_pos((ballStationary[0],ballStationary[1], strokes_1, h1, int(player1_turn), int(player1attacked), int(player2attacked), int(powerUpPlayer1), -1, -1, False))
    player1client.send(sendData)
    #player1client.receive()
    ####################################################################
    ...
    while put and not shoot:  # If we are putting
        ...
        if(player1_turn):
            if(  not(overHole(ballStationary[0], ballStationary[1]))  ):
                ...
                #SEND CLAUSE
                if (line != None):
                    sendData = make_pos((ballStationary[0],ballStationary[1], strokes_1, h1, int(player1_turn), int(player1attacked), int(player2attacked), int(powerUpPlayer1), line[0], line[1],  True))
                else:
                    sendData = make_pos((ballStationary[0],ballStationary[1], strokes_1, h1, int(player1_turn), int(player1attacked), int(player2attacked), int(powerUpPlayer1), -1, -1, True))
                player1client.send(sendData)
                #player1client.receive()
                ####################################################################
                ...
                if(h1 == 0):
                    ...
                    while True:  # Move ball so it looks like it goes into the hole (increase y value)
                        ...
                        ####################################################################
                        #SEND CLAUSE (Zero so we get the aninamtion)
                        if (line != None):
                            sendData = make_pos((ballStationary[0],ballStationary[1], strokes_1, 0, player1_turn, player1attacked, player2attacked, powerUpPlayer1, line[0], line[1], True))
                        else:
                            sendData = make_pos((ballStationary[0],ballStationary[1], strokes_1, 0, int(player1_turn), int(player1attacked), int(player2attacked), int(powerUpPlayer1), -1, -1, True))
                        player1client.send(sendData)
                        #player1client.receive()
                        ####################################################################
    ...
    while shoot:  # If we are shooting the ball
        ...
        if ((overHole(ballStationary[0], ballStationary[1]) == False or overHole(ballStationary2[0], ballStationary2[1]) == False) and (h1 + h2 != 2)):  # If we aren't in the hole
            ...
            if(player1_turn):
                ...
                #SEND CLAUSE
                if (line != None):
                    sendData = make_pos((ballCords[0],ballCords[1], strokes_1, h1, int(player1_turn), int(player1attacked), int(player2attacked), int(powerUpPlayer1), line[0], line[1],  True))
                else:
                    sendData = make_pos((ballCords[0],ballCords[1], strokes_1, h1, int(player1_turn), int(player1attacked), int(player2attacked), int(powerUpPlayer1), -1, -1, True))
                player1client.send(sendData)
                #player1client.receive()
                ####################################################################
